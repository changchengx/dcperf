/* SPDX-License-Identifier: SPDX-License-Identifier: GPL-2.0+
 * Copyright(c) 2025 Fabien Marteau <fabien.marteau@armadeus.com>
 * Copyright(c) 2021 Léo Dumez <leo.dumez@outlook.com>
 */

/* Orig repo: https://github.com/Martoni/pcie_debug
 *
 * Depend:
 ...
 *
 * Build:
 *    $ gcc pci_debug.c -O0 -g3 -Wall -fmessage-length=0 -o pci_debug -lreadline
 */
...
static void
write_8(device_t *dev, unsigned int addr, unsigned char data);

static unsigned char
read_8(device_t  *dev, unsigned int addr);

static void
write_le16(device_t *dev, unsigned int addr, unsigned short int data);

static unsigned short int
read_le16(device_t *dev, unsigned int addr);

static void
write_be16(device_t *dev, unsigned int addr, unsigned short int data);

static unsigned short int
read_be16(device_t *dev, unsigned int addr);

static void
write_le32(device_t *dev, unsigned int addr, unsigned int data);

static unsigned int
read_le32(device_t *dev, unsigned int addr);

static void
write_be32(device_t *dev, unsigned int addr, unsigned int data);

static unsigned int
read_be32(device_t *dev, unsigned int addr);
